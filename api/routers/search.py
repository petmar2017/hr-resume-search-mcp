...
async def search_candidates(
    ...
    """
    Smart candidate search with multiple criteria with Redis caching
    """
    start_time = datetime.utcnow()
    ...
    search_params = {
        ...
        "max_experience_years": search_request.max_experience_years,
    ...
    # Check cache first
    cached_results = cache_service.get_cached_search_results(search_params)
    if cached_results:
        search_metrics.record_cache_hit("search")
        logger.info(f"Cache hit for search: {search_request.search_type.value}")
        
        # Convert cached results back to SearchResponse
        cached_search_results = [
            SearchResult(**result) for result in cached_results["results"]
        ]
        
        return SearchResponse(
            query=search_request.query or "",
            search_type=search_request.search_type,
            total_results=len(cached_search_results),
            results=cached_search_results,
            processing_time_ms=5  # Fast cache response
        )
    
    # Cache miss - record and proceed with database query
    search_metrics.record_cache_miss("search")
    
    # Database query with optimized indexes and caching  
    with search_metrics.track_search_request(search_request.search_type.value, "user"):
        try:
            ...
            # Apply filters based on search type (optimized with indexes)
            if search_request.search_type == SearchType.SKILLS_MATCH:
                if search_request.skills:
                    # Use optimized GIN index for JSONB skills
                    skills_filter = []
                ...
                if search_request.departments:
                    # Use optimized composite index for company+department
                    query = query.join(
                ...
                # Use optimized index on total_experience_years
            ...
            # Apply additional filters with optimized indexes
            ...
            with db_metrics.track_query("COUNT", "search_count"):
            ...
            # Build response with match scoring
            search_results = []
            match_scores = []
            ...
            # Cache the results for future requests (5 minute TTL)
            cache_data = [result.model_dump() for result in search_results]
            cache_service.cache_search_results(search_params, cache_data, ttl=300)
            ...
            logger.info(f"Search completed: {processing_time}ms, {total_results} results, cache stored")
